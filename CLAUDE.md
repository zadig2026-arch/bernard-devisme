...
**Studio navigation**: opening a rubric under its part of the site (Peinture, Sculpture…) shows a two-entry list — « Les œuvres » and « Titre, texte et groupes d'œuvres » — so the rubric's own form is reachable from where Bernard actually looks for it. « Modifier une rubrique » at the bottom of the menu is kept on purpose: he has just learned that path. **Studio readability**: the background stays WHITE — a paper/ink/sienna theme was written on 29/07, then dropped on 01/08 when he answered « je conserve l'affichage blanc ». Don't reintroduce a coloured background. What *is* customised, at his written request of 08/08 ("les icônes + ou … sont très peu visibles, la police est fine"), is the *controls*: `sanity/theme.ts` (colours only — dark top bar, sienna accents, darker greys) and `app/studio/lisibilite.css` (borders, icon size, label weight), the latter imported by the Studio page alone so it never reaches the public site. The CSS targets `data-ui` attributes, never generated classes and never tag names: a column's « + » is an `<a data-ui="Button">`, the row-level « ⋯ » is a `MenuButton` with *no* `aria-label`. Re-verify selectors in the real Studio after any Sanity upgrade.

**Series slug**: `series.slug` builds the public link and is editable by Bernard. On 08/08 he typed a title into it (« Les tentures »), which rendered the rubric unreachable while still listing it. The field now slugifies on generate and refuses a malformed value at publish time; `scripts/fix-series-slugs-2026-08.mjs` sweeps the dataset for any that slipped through. Note GROQ's `match` is wildcard text matching, not regex — filter slugs in JS.
