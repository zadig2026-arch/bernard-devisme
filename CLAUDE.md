...
**Category → series mapping is hardcoded** in `components/series-index.tsx` (`CATEGORIES` array), mirroring the section structure of the original e-monsite. The slug list, order, and titles intentionally match the original site exactly — when adding a new series in Sanity, also add its slug to the right `CATEGORIES` entry, or it falls into the "Autres" bucket. `getCategoryMeta` and `CATEGORY_IDS` are exported from this file for routing use.
